...
export const loadChemicalData = async () => {
  try {
    const response = await fetch('data/cheminfoo.csv');
...
export const normalizeComponents = (components) => {
  ...
  const componentsInMoles = components.map(comp => {
      // Store original input values before converting
      const originalAmount = comp.amount;
      const isMolarInput = comp.isMolar;

      // Calculate moles
      const moles = isMolarInput ? 
          comp.amount : // if already molar, use directly
          comp.amount / comp.molecularWeight; // if mass basis, convert to moles

      return {
          ...comp,
          moles,
          originalAmount,
          originalBasis: isMolarInput ? 'molar' : 'mass'
      };
  });
  ...
  return componentsInMoles.map(comp => ({
      ...
      vaporPressureConstants: comp.vaporPressureConstants,
      originalAmount: comp.originalAmount,
      originalBasis: comp.originalBasis,
      amount: (comp.moles / totalMoles) * 100, // convert to percentage
      isMolar: true, // now everything is in mole fraction
      moles: comp.moles // keep moles for potential future calculations
  }));
}